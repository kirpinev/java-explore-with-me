...
import ru.practicum.ewm.events.service.EventConstants;
...
public class RequestServiceImpl implements RequestService {
    ...
    private Request getRequestByRequesterIdAndId(Long userId, Long requestId) {
        ...
        if (request == null) {
            throw new NotFoundException(String.format(RequestConstants.REQUEST_NOT_FOUND_EXCEPTION_MESSAGE,
                    requestId));
        }
        return request;
    }
    ...
    private void validateEventInitiator(Long userId, Event event) {
        if (event.getInitiator().getId().equals(userId)) {
            throw new OperationException(RequestConstants.REQUEST_SAME_USER_ID_EXCEPTION_MESSAGE);
        }
    }

    private void validateEventState(Event event) {
        if (!event.getState().equals(State.PUBLISHED)) {
            throw new OperationException(RequestConstants.REQUEST_STATE_EXCEPTION_MESSAGE);
        }
    }

    private void checkParticipantLimit(Event event, List<Request> requests) {
        if (requests.size() >= event.getParticipantLimit()) {
            throw new OperationException(RequestConstants.REQUEST_LIMIT_EXCEPTION_MESSAGE);
        }
    }
    ...
    private Event findEventById(Long eventId) {
        return eventRepository.findById(eventId).orElseThrow(() -> {
            throw new OperationException(String.format(EventConstants.EVENT_NOT_FOUND_MESSAGE, eventId));
        });
    }
    ...
    private Request getRequestById(Long id) {
        return requestRepository.findById(id).orElseThrow(() -> {
            throw new NotFoundException(String.format(RequestConstants.REQUEST_NOT_FOUND_EXCEPTION_MESSAGE, id));
        });
    }
